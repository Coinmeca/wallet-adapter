...
export const useWallet = create<WalletStore & WalletStoreAction>((set) => ({
    ...
    update: (wallet: Wallet, chain?: Chain) => set((state: WalletStore & WalletStoreAction) => ({ ...state, info: { address: wallet?.address, provider: wallet?.provider ?? state?.info?.provider }, provider: wallet?.provider ?? state?.info?.provider, address: wallet.address, chain: chain ?? state?.chain })),
